...
Here are summaries of each of the model's out-of-sample performances:
...
```{r eval=TRUE}
preds = vector(mode='list', length=length(model.fnames))
labels = vector(mode='list', length=length(model.fnames))
for(i in 1:length(model.fnames)){
  label.name = gsub('models/', '', gsub('_svm.*$', '', model.fnames[i]))
  load(model.fnames[i])
  load(container.fnames[i])
 results = classify_model(container, model$SVM)
 preds[[i]] = ifelse(results$SVM_LABEL==1, results$SVM_PROB, 1-results$SVM_PROB)
 labels[[i]] = container@testing_codes
}

preds = do.call(cbind, preds)
colnames(preds) = paste0(label.names, '_predprobs')
labels = do.call(cbind, labels)
colnames(labels) = label.names
d = as.data.frame(cbind(preds, labels))
...
```{r}
for(i in 1:length(label.names)){
  roc_curve = AUC::roc(preds[,i], labels[,i])
  plot(roc_curve)
}
```
